...
\begin{itemize}
  ...
  $\sqrt{S^2 / \sigma^2} \sim \chi_{n-1}^2$. %TODO: Fix the error here.
\end{itemize}
\begin{theorem}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \theta)}{m(\bm x)}$$
    where $m(\bm x) = \int f(\bm x | \bm \theta) \pi(\bm \theta) \ud \bm \theta
    $
   \end{itemize}
...
\right]^2\right\}}$$
\textbf{Note} If $W(\bm X)$ is unbiased for $\theta$, then
$$\left[\frac{\ud}{\ud \theta} E_\theta(W(\bm X))\right]^2 = 1 \text{ and }
Var_\theta(W(\bm X)) \geq \frac{1}{E\left\{\left[ \frac{\ud}{\ud \theta} ln(f(\bm x | \theta))
\right]^2\right\}}$$
...
$$cov(X, Y)^2 \leq Var(X)Var(Y) ~ (1)$$
where $X=W(\bm X)$ and $Y=\frac{\ud}{\ud \theta} ln[f(\bm X | \theta)]$.

First note that
$$(2)~ \frac{\ud}{\ud \theta}E_\theta(W(\bm X)) = \int_{\mathcal
X}\frac{\ud}{\ud \theta} W(\bm X) f(\bm X | \theta) \ud \bm x ~ \text{ condition (*)}$$
We multiply this by $1 = \frac{f(\bm X | \theta)}{f(\bm X | \theta)}$ and
...
\bm X}\right] ~ (3)$$
If we consider here $X=W_\theta(\bm X)$ and $Y=\frac{\ud ln(f(\bm X | \theta))}{\ud
\bm X}$, we have
...
Also, because of (4), we also have
$$(5) ~ Var_\theta\left[\frac{\ud}{\ud \theta} ln(f(\bm X | \theta))\right] =
\underbrace{E\left[\left(\frac{\ud}{\ud \theta}ln(f(X))
...
$$=\frac{\left[\frac{\ud}{\ud \theta} E(W(\bm X))\right]^2}{E\left\{ \left[
 \frac{\ud}{\ud \theta} ln(f(X))\right]^2 \right\}}$$
\end{theorem}
\begin{corollary}
\label{corollaryCR1}
...
In the previous example we found that $Var(\bar X) = \frac{n}{\lambda}$. It is
called the Cramer-Rao lower bound. Therefore $\bar X$ is the best unbiased
estimator of $\lambda$.
\end{example}
...
\end{itemize}
\begin{example}
R-function mle() computes this approximation numerically. Recall the example
\ref{exampleMLESpatial1} which used R-script \verb#MLESpatial.R#. 

Function ccov(sol) returns this approximation
$$\bm \theta = \begin{bmatrix}
...
-0.58 & -1.03 & 1.00
\end{bmatrix}$$
Using function summary(sol) gives results
\begin{table}[h]
\caption{Correlations between the random variable pairs
of example \ref{exampleSpatialData1}}
\label{exampleSpatialData1Table1}
