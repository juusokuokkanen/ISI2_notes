...
\item Let $g(\bm x)$ be a transformation $g: \ureal^n \to \ureal$. Then, the
	...
	E(g(\bm X)) = \left\{
	\begin{array}{ll}
	\int_{-\infty}^\infty \ldots \int_{-\infty}^\infty g(\bm x) f(x) \ud x_1 \ldots
	\ud x_n & \text{if $\bm X$ is continuous} \\
...
replicate provides an element to $\bm X_1$, which is a rather long vector.
...
\end{array} \right. \\
& = p^{x_i}(1-p)^{1-x_i} & 0 < p < 1 \text{ or } 0 \leq p \leq 1
\end{array}
\end{equation*}
...
\begin{itemize}
  ...
  we just need to find the practical derivatives of $ln(L)$ (or L) with respect
