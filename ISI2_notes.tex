...
$$P(a < X \leq b) = F(b) - F(a)$$
\item [\bf{Note:}] Be careful with $<$ and $\leq$ when working with discrete
random variables.
\item The probability density function (pdf for short) is defined for a
continuous random variable $X$ as
$$f_X(x) = F'(x) = \frac{\ud F_X(x)}{\ud x}, ~ -\infty < x < \infty$$
and
$$\int_{-\infty}^x f_X(t)\ud t = F_X(x)$$
\item The probability mass function (pmf for short) is defined for a discrete
...
\begin{itemize}
  ...
  \item If $g(x)$ is an increasing function of $x$, then
  $$F_Y(y) = F_X(g^{-1}(y))$$
...
\begin{itemize}
  ...
  $f_{X,Y}(x,y)$ as
  $$P((X,Y) \in A) = \iint\limits_A f_{X,Y}(x,y) \ud x \ud y$$
...
\renewcommand{\arraystretch}{1.6}
\begin{array}{ll}
f_{X,Y}(y \mid x)  = \frac{f_{X,Y}(x,y)}{f_X(x)} & \text{(for both discrete and
...
\begin{itemize}
  ...
  \begin{equation*}
	...
	it does not necessarily mean that $X$ and $Y$ are independent.
\end{itemize}
\section{Random vectors}
\begin{itemize}
  \item Random vectors generalize the bivariate random variables to the
  $n$-variate case.
  	\begin{equation*}
	\renewcommand{\arraystretch}{1.6}
	\begin{array}{ll}
	...
	\renewcommand{\arraystretch}{1.6}
	\begin{array}{ll}
	P(\bm X \in A) = \substack{\sum \ldots \sum\\A} f(\bm x)
	\end{array}
	\end{equation*}
	where $f(\bm x)$ is a joint pmf.

\item Let $g(\bm x)$ be a transformation $g: \ureal^n \to \ureal$. Then, the
	expected value is
	\begin{equation*}
	\renewcommand{\arraystretch}{1.6}
	E(g(\bm X)) = \left\{
	\begin{array}{ll}
	\int_{-\infty}^\infty \ldots \int_{-\infty}^\infty g(\bm x) f(\bm x) \ud x_1
	\ldots \ud x_n & \text{if $\bm X$ is continuous} \\
	\mathop{\sum\ldots\sum}_{\bm X \in \ureal^n}g(\bm x)f(\bm x)& \text{if $\bm X$
	...
	\begin{itemize}
	\item The joint pdf
	of $\bm X$ can be written equivalently as
	...
	\item The conditional pdf/pmf for $\bm X_1 \mid \bm X_2$ is
	$$f(\bm X_1 \mid \bm X_2) = \frac{f(\bm X_1, \bm X_2)}{f(\bm X_2)}$$
	
	%TODO: Left out plots and table visualization from notes 2 page 2 (end), are
	% they needed?
	
	\item Expected value of a random vector is a vector
...
\begin{bmatrix}
1 & corr(X_1, X_2) & \hdots & corr(X_1, X_n) \\
corr(X_2, X_1) & 1 & \hdots & corr(X_2, X_n) \\
\vdots & \vdots & \ddots  & \vdots \\
corr(X_n, X_1) & corr(X_n, X_2) & \hdots & 1 \\
\end{bmatrix}
\end{array}
\end{equation*}
...
Var(X) = E_Y\left[Var_{X\mid Y}(X \mid Y)\right]+Var_Y\left[E_{X\mid Y}(X \mid
Y)\right]
\end{array}
\end{equation*}
...
\end{array}
\end{equation*}
{\bf Note: } These equations need to be remembered by heart!
...
and the marginal pdf/pmf of each $X_i$ is the same function $f(x)$.
Alternatively, $X_1,\ldots,X_n$ are called {\it independent and identically
...
$$f(x_1,\ldots,x_n) = f(x_1)f(x_2)\ldots f(x_n) = \prod_{i=1}^n f(x_i)$$
...
\end{equation*}
\end{example}
A much simpler solution: notice that $P(X_i > 2) = 1 - F(2) =
e^{-2/\beta}$. Because $X_i$'s are independent, then also events $(X_i > 2)$ are
...
replicate provides an element to $\bm X_1$, which is a rather long vector (e.g.
...
Let $\bm X_1 = \begin{bmatrix} X(u_1) & X(u_2) & X(u_3) \end{bmatrix}^T$ include
...
The joint pdf of $\bm X_1$ is 3 variate normal distribution with mean $\bm \mu$
and variance $\bm \Sigma$ 
$$f(\bm x_1 \mid \bm \mu, \bm \Sigma) = \frac{1}{\sqrt{(2\pi)^3}}
...
distributed. In the case of random vectors, this is specified by saying that the
...
Random samples can be summarized to a well defined summary called
{\it statistic}.
...
\end{array}
\end{equation*}
\ldots and sample mean, sample variance, sample standard deviation,
sample quartiles etc.
...
simulation. Script repeats the sampling from Exponential distribution $M=10000$
...
\begin{lemma} Let $X_1,\ldots,X_n$ be a random sample form a population and let
$g(X)$ be a function such that $E(g(X_i))$ and $Var(g(X_i))$ exist. Then
$$E\left[\sum_{i=1}^ng(X_i)\right] = n\cdot E(g(X_1))$$
and
$$Var(\sum_{i=1}^n g(X_i)) = n \cdot Var(g(X_1))$$
\textbf{Proof} 
...
\end{theorem}
\begin{itemize}
  \item Proof for a and b are familiar from ISI1 course. Proof of c applies the theorem
...
$S^2$ is an unbiased estimator of population variance $\sigma^2$, because
$E(S^2) = \sigma^2$.
\end{itemize}
\begin{example}
...
X_i$, where $X_i$'s do not need to be identically distributed, but they are
...
\ref{theoremConvolutionFormula1} again to find the pdf of $Z + X_3$. This is
not trivial in very general case, but there are easier ways to do this, e.g. when
...
of freedom
\begin{itemize}
  \item[a)] If $Z$ is a $N(0,1)$ random variable, then $Z^2 \sim \chi_1^2$, that
  is, the square of a standard normal random variable is a chi-squared random
  variable. %TODO: Is Z^2 \chi_p^2 or \chi_1^2?
  \item[b)] If $X_1,\ldots,X_n$ are independent and $X_i \sim \chi_{p_i}^2$ then
...
\end{lemma}
\begin{example}
Illustrate the result of lemma \ref{lemmaChiRV1} using R-script. In file
...
where $\mu = E(\bar X)$ and $\sigma / \sqrt{n} = sd(\bar X)$.
\begin{itemize}
  \item This transformation could be used to make inference about $\mu$ using
  the observed $\bar x$, if $\sigma^2$ was known.
  \item However, that is not the case, and therefore we use $\frac{\bar X -
  \mu}{S / \sqrt{n}}$ which can also be written as
  $$\frac{(\bar X - \mu)/(\sigma / \sqrt{n})}{\sqrt{S^2 / \sigma^2}}$$
  where $(\bar X - \mu)/(\sigma / \sqrt{n}) \sim N(0,1)$ and the denominator is square root of scaled chi-square random variable: $\sqrt{\chi^2_{n-1}/(n-1)}$.
\end{itemize}
\begin{theorem}
...
\textbf{Illustration:} R-script \verb#normalchi.R#
\begin{itemize}
  \item We use samples of sizes $3,4,\ldots,100$ and $M=10000$, where $X\sim
  N(5,3^2)$.
  ...
  $N(0,1)$ and $t_{n-1}$ vanishes as $n \to \infty$.
\end{itemize}
...
\textbf{Note:} \begin{itemize}
  ...
  \item[b)] If $X \sim t_q$, then $X^2 \sim F_{1,q}$ (recall that if $X \sim
...
\end{array}
\end{equation*}
Because $P(X \geq 0)=1$, both terms are positive and
\begin{equation*}
\renewcommand{\arraystretch}{1.6}
\begin{array}{ll}
E(X) & \geq \sum_{\mathcal X \geq t}xf(x) \geq \sum_{\mathcal X \geq t} t f(x) =
t \underbrace{\sum_{X \geq t} f(x)}_{P(X\geq t)} \\
& = t \cdot P(X\geq t) \\
& \\
...
$$P(\abs{X - E(X)} \geq t) = P(Y \geq t^2) \leq \frac{E(Y)}{t^2}~\text{ (use
Markov's inequality)}$$ $$P(\abs{X - E(X)} \geq t) \leq \frac{Var(X)}{t^2}$$
\end{theorem}
...
F(c_1).$ Wether $x$ belongs to a class $[c_1, c_2[$ is a Bernoulli($p$)
distributed random variable with
$$p = E(Y) = F(c_1) - F(c_2)$$

\textbf{Note} Another version of the law of large numbers is the strong
law of strong numbers (SLLN).
$$P(\lim_{n \to \infty} \bar{X}_n = \mu) = 1.$$
$\bar{X}$ converges to $\mu$ with probability 1, meaning {\it almost sure
convergence} (melkein varma konvergointi).
\end{theorem}
\begin{definition}
A sequence of random variables converges in distribution (jakaumakonvergenoi) to
...
\leq 1 - \epsilon)$$
Because the sample is i.i.d.
\begin{equation*}
\renewcommand{\arraystretch}{1.6}
\begin{array}{lll}
P(X_{(n)} \leq  1 - \epsilon) & = P(\text{All } X_i \leq 1 - \epsilon) \\
...
$$P\left(X_{(n)} \leq 1 - \frac{t}{n}\right) = \left(1-\frac{t}{n}\right)^n$$
...
1 - X_{(n)} \geq \frac{t}{n} & | \cdot n \\
n(1 - X_{(n)}) \geq t \\
P(n(1 - X_{(n)}) \geq t) = e^{-t} \\
P(n(1 - X_{(n)}) \leq t) = \underbrace{1 - e^{-t}}_{\text{Exponential(1) cdf}}
\\
\end{array}
\end{equation*}
...
each sample, find $X_{(n)}$ and compute $y=n(1-X_(n))$. Plot the histogram of
...
\sum_{i=1}^n X_i$. Let $G_{n}(z)$ denote the cdf of
$$Z_n = \frac{\sqrt{n}(\bar{X}_n-\mu)}{\sigma} =
\frac{\bar{X}_n-\overbrace{\mu}^{E(\bar{X}_n)}}{\underbrace{\sigma/\sqrt{n}}_{sd(\bar{X}_n)}}$$
Then $$\lim_{n \to \infty}G_{n}{z} = \underbrace{\int_{-\infty}^z
\underbrace{\frac{1}{\sqrt{2\pi}}e^{-y^2/2}\ud y}_{N(0,1) \text{
pdf}}}_{N(0,1) \text{ cdf}}$$
That is: $Z_n$ has limiting standard normal distribution: $Z_n
\xrightarrow{d} N(0,1)$. Equivalently, $\bar{X}
\xrightarrow{d} N(\mu,\sigma^2/n)$.

\textbf{Notes:}
\begin{itemize}
  \item We assume only finite $\mu$ and $\sigma^2$ and end up to normality.
  \item The rate of convergence is affected by the original distribution of
  $X_i$
  \begin{itemize}
    \item The closer the distribution of $X_i$ is the Normal curve, the faster
    the convergence is.
    \item The rate needs to be evaluated case by case.
  \end{itemize}
  \item Provides an all purpose approximation of the distribution of sums of
...
proportion should be close to 0.05 if the approximation is good. This is left as
an exercise.
...
be more slow than when $\sigma$ is known. Demonstration is left as an exercise.
...
The model can be of the following form
$$Y_i = \mu(x_i) + E_i$$
...
$$Y_i \sim Bernoulli(p) ~\text{ and } \theta = p$$
...
$\left[0,1\right]$, a better model uses some other function for
$p(\text{Age}_i)$.
Most commonly we use
$$log\left(\frac{p_i}{1-p_i}\right) = \beta_0 + \beta_1 \text{Age}_i$$
Which is a logit transformation $[0,1] \rightarrow \ureal$. This leads to binary
logistic regression.
...
%That is, if we have the complete data and you know only the sample mean, we
% both still have all information on $\mu$ the sample includes. However, if a
% third
...
\begin{itemize}
  \item If $\bm X$ is a discrete random vector, then
  $$L(\bm \theta | \bm X) = P_{\bm \theta}(\bm X = \bm x)$$
...
no sense to specify a pdf for it. In Bayesian statistics however, $\theta$ is
...
\end{definition}
\begin{itemize}
  \item We first consider the methods to find estimators and there after
  ...
  \end{itemize}
  \item Often, there is a natural, intuitive candidate for a point estimator,
...
The approach also generalizes to cases where $E(Y_i)$ is a function of a fixed
variable $x_i$. Recall the model $Y_i = \beta_0 + \beta_1 x_i + E_i$,
where $Y_i$ is a random variable and $x_i$ is another fixed observable variable.
We can specify $\beta_0$ and $\beta_1$ %TODO: How was this sentense suposed to
% be formed?
such that $E(E_i) = 0$ and 
$$E(Y_i) = E(\underbrace{\beta_0 + \beta_1 x_i}_{\text{fixed}} +
\underbrace{E_i}_{E(E_i) = 0}) = \beta_0 + \beta_1 x_i = E(Y_i
| x_i) = \mu(x_i, \bm \beta)$$
...
The solutions are
$$\widehat{\beta}_1 = \frac{\sum(x_i - \bar x)(y_i - \bar y)}{\sum(y_i -
\bar y)^2} \text{ and } \widehat{\beta}_0 = \bar y - \widehat{\beta}_1 \bar x$$
...
\begin{itemize}
  ...
  \bm E = \begin{bmatrix} E_1 \\ \vdots \\ E_n
  \end{bmatrix}, ~$$ 
  and then the model is $\bm Y = \bm X'\bm \beta + \bm \epsilon$. Now $\bm X$ is
  a matrix of fixed values. $\bm Y$ and $\bm E$ are random vectors.
  \item The least squares estimator of $\bm \beta$ is found by minimizing
  $$\bm E' \bm E = (\bm y - \bm X' \bm \beta)'(\bm y - \bm X' \bm
  \beta)$$
  \item The solution is $\widehat{\bm \beta} = (\bm X' \bm X)^{-1}\bm X' \bm y$
  \item This is a general solution for the multiple regression case, where the
  number of columns in $\bm X$ is higher than 2.
\end{itemize}
...
We get 
$$\tilde \mu = \bar X$$
$$\tilde \sigma^2 = \frac{1}{n}\sum X^2 - \bar X^2 =\frac{1}{n}\sum(X_i - \bar
X)^2$$

\textbf{Note:}
$$\tilde \sigma^2 = \underbrace{\frac{1}{n}\sum(X_i - \bar X)^2}_{\text{Biased
...
Here $a$ is called the shape parameter and $b$ is called the scale parameter.
...
form, but can be well approximated using function \verb#gamma# in R.
...
estimates of $a$, $b$ numerically using R-function \verb#NRnum#.
...
% TODO: How to get shortbox working?
\begin{shortbox}
...
\begin{itemize}
  ...
  approximate $f(a)$ by a linear function. Find $a$ such that the approximation
  is 0.
   \item[2.] Set the solution as $a'$ and repeat step 1.
\end{itemize}
If the initial guess $a'$ was good enough and $f$ is continuous and
differentiable, we will find good approximate of the solution within
few iterations.

Gauss-Newton generalized this to vector valued $\bm a$ and $\bm x$. The
algorithm has been implemented in R-function \verb#NRnum# of package
\verb#lmfor#.
The function evaluates also the required derivatives $\frac{\ud f(\bm x)}{\ud
\theta_i}$ numerically.
\end{shortbox}
...
\theta_1 \\ \vdots \\ \theta_k
\end{bmatrix}$$
For i.i.d. data $X_1, \ldots, X_n$ from population $f(x |
\theta)$, the likelihood simplifies to
$$L(\bm \theta | \bm x) = \prod_{i=1}^n f(x_i | \bm \theta)$$
\begin{definition}
...
Let $X_1,\ldots,X_n$ be i.i.d. from $N(\theta,1)$, meaning that only the
population mean $\theta$ is unknown.
\begin{equation*}
\renewcommand{\arraystretch}{1.7}
\begin{array}{rl}
...
coincide (are at the same point $\bm \theta$) because $L(\bm \theta | \bm x)
\geq 0$ and $ln$-function is increasing on $(0, \infty)$.
\end{itemize}
...
\textbf{Note} \verb#mle# gives only the candidate of MLE, so you are responsible
for checking that it really is the MLE.
\end{example}
...
\begin{itemize}
  ...
  we just need to find the partial derivatives of $ln(L)$ (or L) with respect
...
\end{example}
\begin{example}
\label{exampleMLESpatial1} 
Example ML6 (MLE in dependent data)
...
$\bm Y = \begin{bmatrix} Y_1 \\ \vdots \\ Y_{25} \end{bmatrix}$
is generated by following model
$$Y \sim N(\bm \mu, \bm \Sigma), \text{ where } 
\bm \mu = \begin{bmatrix} \mu_1 \\ \vdots \\ \mu_{25} \end{bmatrix},
\mu_i = 0 \text{ for all }i \text{, and } Var(Y_i) = \sigma_i^2 = 2^2 \text{
for all }i$$
$$corr(Y_i, Y_j) = exp^{-\phi s_ij} \text{, where } \phi = \frac{1}{2}$$
and $s_{ij}$ is the euclidian distance between the location $i$ and $j$,
$s_{ij} = ||u_i - u_j||$.
...
in R-function \verb#nll#. The script provides following resulting estimates.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \theta)}{m(\bm x)}$$
    where $m(\bm x) = \int_{\bm \Theta} f(\bm x | \bm \theta) \pi(\bm \theta)
    \ud \bm \theta $ and $\bm \Theta$ is the sample space of $\bm \theta$.
   \end{itemize}
...
\left[{n\choose
y}p^y(1-p)^{n-y}\right]\left[\frac{\Gamma(\alpha+\beta)}{\Gamma(\alpha)\Gamma(\beta)}p^{\alpha-1}(1-p)^{\beta-1}\right]$$
...
\textbf{Note} The Beta distribution is said to be the {\it conjugate prior} for
the Binomial distribution, because the posterior distribution belongs to the same family. The
...
\begin{itemize}
  ...
  $$\pi(\bm \theta | \bm x) = \frac{f(\bm x | \bm \theta)\pi(\bm \theta)}{m(\bm
  x)}$$
  the denominator $m(\bm x) = \int f(\bm x | \bm \theta) \pi(\bm \theta) \ud
  \bm \theta$ is hard to compute in general case.
  \begin{itemize}
    \item The modern computational methods, so called {\it Markov chain Monte
    Carlo (MCMC)} methods allow simulation of observations from $\pi(\bm \theta
    | \bm x)$ without the need to evaluate $m(\bm x)$.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item[2.3] If $\alpha \geq 1$, then accept the proposal and set
      $\theta_{t+1}=\theta'$. If $\alpha < 1$, then accept $\theta'$ as
      $\theta_{t+1}$ with probability $\alpha$.
...
\end{bmatrix}$ from a normally distributed population. The aim is to estimate
the population mean $\theta$.
We use restrictive prior for it:
...
\end{definition}
\begin{itemize}
  \item In general, any increasing function of the absolute distance
  $\abs{W-\theta}$ is a reasonable measure for the quality of $W$ (e.g. mean
  absolute error $E_\theta(W-\theta)$), but MSE is 1) mathematically tractable
...
meaning $\hat \sigma^2$ is a biased estimator of $\sigma^2$.
...
\begin{itemize}
  ...
  parameters, but not of scale parameters.
\end{itemize}
\end{example}
...
unbiased estimators. In that class
$$E_\theta(W-\theta)^2 = Var_\theta(W)$$
Therefore, form among unbiased estimators, we should select the one with the
lowest variance.
\begin{definition}
An estimator $W^*$ is the { \it best unbiased estimator (BUE)} of
$\tau(\theta)$, if it satisfies $E_\theta(W^*)=\tau(\theta)$ for all $\theta$ and for any other
estimator $W$ with $E_\theta(W)=\tau(\theta)$ we have
$$Var_\theta(W*) \leq Var_\theta(W) \text{ for all } \theta$$
$W^*$ is also called { \it uniform minimum variance unbiased estimator} of
$\tau(\theta)$ (UMVUE).
\end{definition}
\begin{example}
\label{exampleBestEstimator1}
...
Conclusion however will be, that $Var_\lambda(\bar X) \leq Var_\lambda(S^2)$ for
all $\lambda$. So, from among these two candidates, $\bar X$ should be selected.
But is $\bar X$ really the best?

We could construct infinitely many unbiased estimators as weighted mean of $\bar
X$ and $S^2$:
$$W_w = w \bar X + (1-w)S^2 \text{ where } w \in [0,1]$$
Could some selection of $w$ lead to smaller variance than $Var(\bar X) =
\frac{\lambda}{n}$? Or is there some other estimator that has $Var(W_w) =
\frac{\lambda}{n}$?
% TODO: Did I miss something here
The following theorem gives light to this function.
%TODO: Left out the $\hat \lambda = \bar X$ at notes 12 page 8 end. Is it
% needed?
\end{example}
\begin{theorem} (Cramer-Rao Inequality)

Let $X_1,\ldots,X_n$ be a sample with pdf $f(\bm X | \theta)$ and let $W(\bm X)
= W(X_1,\ldots,X_n)$ be any estimator satisfying
$$(*) ~ \frac{\ud}{\ud \theta}E_\theta(W(\bm X)) = \int_{\mathcal
X}\frac{\ud}{\ud \theta} W(\bm x) f(\bm x | \theta) \ud \bm x$$
and
$$Var_\theta(\bm X) < \infty $$
then
$$Var_\theta(W(\bm X)) \geq \frac{\left[\frac{\ud}{\ud \theta} E_\theta(W(\bm
X))\right]^2}{E\left\{\left[ \frac{\ud}{\ud \theta} ln(f(\bm x | \theta))
\right]^2\right\}}$$
\textbf{Note} If $W(\bm X)$ is unbiased for $\theta$, then $E_\theta(W(\bm X))=
\theta$ and  
$$\left[\frac{\ud}{\ud \theta} E_\theta(W(\bm X))\right]^2 = 1
\text{ and } Var_\theta(W(\bm X)) \geq \frac{1}{E\left\{\left[ \frac{\ud}{\ud \theta} ln(f(\bm x | \theta))
\right]^2\right\}}$$
\textbf{Note} Integral in condition (*) can be replaced by sum to apply the
theorem to discrete random variables.

\textbf{Proof: } Based on Cauchy-Schwarz inequality
$$\left[cov(X, Y)\right]^2 \leq Var(X)Var(Y)$$
where $X=W(\bm X)$ and $Y=\frac{\ud}{\ud \theta} ln[f(\bm X | \theta)]$.
Rearrange it as
$$Var(X) \geq \frac{\left[cov(X,Y)\right]^2}{Var(Y)}, ~ (1)$$

First note that
% Old version
%$$(2)~ \frac{\ud}{\ud \theta}E_\theta(W(\bm X)) = \int_{\mathcal
%X}\frac{\ud}{\ud \theta} W(\bm X) f(\bm X | \theta) \ud \bm x ~ \text{
% condition (*)}$$ We multiply this by $1 = \frac{f(\bm X | \theta)}{f(\bm X | \theta)}$ and
%utilize $\frac{\ud ln(f(x))}{\ud x} = \frac{1}{f(x)}f'(x) =
%\frac{\frac{\ud}{\ud x}f(x)}{f(x)}$ to get 
%$$= \int_{\mathcal X}W(\bm X)\frac{\frac{\ud}{\ud \theta} f(\bm X |
%\theta)}{f(\bm X | \theta)} f(\bm X | \theta) \ud x $$
%Which we can write as an expected value of transformation
%$$ = E_\theta\left[W_\theta(\bm X) \frac{\ud ln(f(\bm X | \theta))}{\ud
%\bm X}\right] ~ (3)$$
%If we consider here $X=W_\theta(\bm X)$ and $Y=\frac{\ud ln(f(\bm X |
%% \theta))}{\ud \bm X}$, we have
%$$E(Y) = E\left[\frac{\ud}{\ud \theta}ln(f(\bm X | \theta ) \right]$$
%and if we assume that $X = W_\theta(\bm X) = 1$, and utilize the equation (3),
%we get 
%$$ = E\left[1\cdot \frac{\ud}{\ud \theta} ln(f(\bm X | \theta))\right]$$
%we can write this equation then in the form of equation (2)
%$$=\frac{\ud}{\ud \theta} E_\theta(X) = \frac{\ud}{\ud \theta} 1  = 0 ~
%(4)$$
%Now, $E_\theta\left[W_\theta(\bm X) \frac{\ud ln(f(\bm X | \theta))}{\ud
%\bm X}\right] = cov(W(\bm X), \frac{\ud}{\ud \theta}ln(f(\bm X | \theta ))$.

%New version
\begin{equation*}
\renewcommand{\arraystretch}{1.7}
\begin{array}{lll}
(2)~\frac{\ud}{\ud\theta}E_\theta(W(\bm X)) & = \int_{\mathcal X}
\frac{\ud}{\ud \theta}W(\bm x) f(\bm x | \theta) \ud \bm x \\
& = \int_{\mathcal X} W(\bm x)\frac{\ud}{\ud \theta} f(\bm x | \theta) \ud \bm
x\\
& = \int_{\mathcal X} W(\bm x)\frac{\ud}{\ud \theta} f(\bm x | \theta)
\underbrace{\frac{f(\bm x | \theta)}{f(\bm x | \theta)}}_{=1} \ud \bm x\\
& = \int_{\mathcal X} W(\bm x) \frac{\frac{\ud}{\ud \theta}f(\bm x |
\theta)}{f(\bm x | \theta)} f(\bm x | \theta) \ud \bm x\\
& = E_\theta\left[W(\bm X) \frac{\frac{\ud}{\ud \theta}f(\bm X |
\theta)}{f(\bm X | \theta)} \right] & \left\| ~ \frac{\ud ~ ln(f(\bm
X))}{\ud \bm X} = \frac{f'(\bm X)}{f(\bm X)} \right.\\
& = E_\theta\left[W(\bm X) \frac{\ud ~ ln(f(\bm
X | \theta))}{\ud \theta} \right] ~ (3)
\end{array}
\end{equation*}
...
Use this with $X=W(\bm X)$ and $Y=\frac{\ud}{\ud \theta}ln\left(f(\bm X
| \theta)\right)$. 

Now
$$E(Y) = E\left[\frac{\ud}{\ud \theta}ln\left( f(\bm X | \theta)
\right)\right]$$
$$=E\left[\underbrace{1}_{W(\bm X)} \frac{\ud}{\ud \theta}
ln\left(f(\bm X | \theta \right)\right]$$ 
$$=\frac{\ud}{\ud \theta}E_\theta(1) = 0, ~ (4)$$
where we just applied (3) in the reverse direction. 

Therefore
$$cov\left( W(\bm X), \frac{\ud~ln(f(\bm X | \theta))}{\ud \theta} \right) =
E_\theta\left( W(\bm X) \frac{\ud}{\ud~ ln(f(\bm X | \theta))} \right)=
\frac{\ud}{\ud \theta}\left(E_\theta(W(\bm X))\right)$$ Also, because of (4),we
have $$(5) ~ Var_\theta\left[\frac{\ud}{\ud \theta} ln(f(\bm X | \theta))\right] = \underbrace{E\left[\left(\frac{\ud}{\ud \theta}ln(f(X))
\right)^2\right]}_{Var(Y) = E(Y^2) - \underbrace{\left[E(Y)\right]^2}_{=0
\text{ because of (4)}}=E(Y^2)}$$
Using (5) and (2) in (1) gives
$$Var_\theta(W(\bm X)) \geq \frac{\left[cov\left(W(\bm X),
\frac{\ud}{\ud \theta} ln(f(\bm X |
\theta))\right)\right]^2}{Var\left[\frac{\ud}{\ud \theta} ln(f(\bm X |
\theta)) \right]}$$
$$=\frac{\left[\frac{\ud}{\ud \theta} E(W(\bm X))\right]^2}{E\left\{ \left[
 \frac{\ud}{\ud \theta} ln(f(\bm X))\right]^2 \right\}}$$
\end{theorem}
\begin{corollary}
\label{corollaryCR1}
(Cramer-Rao i.i.d. case)
$$Var(W(\bm X)) \geq \frac{\left[\frac{\ud}{\ud \theta} E(W(\bm
X))\right]^2}{n \cdot E_\theta\left\{ \left[ \frac{\ud}{\ud \theta} ln(f(X_i
| \theta ))\right]^2 \right\}}$$
\textbf{Note} The quantity $E_\theta\left\{ \left[ \frac{\ud}{\ud \theta}
ln(f(\bm X | \theta ))\right]^2 \right\}$ is called the information number or
Fisher's information.

\textbf{Note}  Any candidate estimator satisfying $E_\theta[W(\bm X)] =
\tau(\theta)$ and attaining the Cramer-Rao bound is the best unbiased estimator
of $\tau(\theta)$.
\end{corollary}
\begin{lemma}
\label{lemmaCB1}
If $f(x | \theta)$ satisfies
$$\frac{\ud}{\ud \theta} E_\theta\left[\frac{\ud}{\ud \theta} ln(f(x |
\theta))\right] = \int \frac{\ud}{\ud \theta}\left\{
\left[\frac{\ud}{\ud \theta} ln(f(x | \theta))\right]f(x | \theta)\right\} \ud
x ~ (*)$$
Then the denominator of Cramer-Rao bound becomes
$$E_\theta\left\{ \left[ \frac{\ud}{\ud \theta} ln(f(\bm x |
\theta))\right]^2\right\} = -E_\theta\left\{ \frac{\ud^2}{\ud \theta^2}
ln(f(\bm x | \theta))\right\}$$
Condition (*) is satisfied at least by the members of exponential family (e.g.
Bernoulli, Binomial, Poisson, exponential, normal, gamma\ldots).
\end{lemma}
\begin{example} (Poisson case)

The i.i.d. Poisson case where $\theta = \lambda$. Numerator of the Cramer-Rao
bound is

$$\frac{\ud}{\ud \lambda} E_\lambda(\bar X) = \frac{\ud}{\ud \lambda} \lambda
= 1$$
Denominator (of corollary \ref{corollaryCR1}) becomes
$$nE_\lambda\left[\left( \frac{\ud}{\ud x} ln(f(x | \lambda)) \right)^2\right]$$
By using lemma \ref{lemmaCB1}

\begin{equation*}
\renewcommand{\arraystretch}{1.7}
\begin{array}{ll}
=-nE_\lambda\left\{ \frac{\ud^2}{\ud \lambda^2}
ln(f(\bm x | \lambda))\right\} \\
=-nE_\lambda\left\{ \frac{\ud^2}{\ud \lambda^2}
ln(\frac{e^{-\lambda}\lambda^x}{x!})\right\}\\
=-nE_\lambda\left\{ \frac{\ud^2}{\ud \lambda^2}
-\lambda +x \cdot ln(\lambda)-ln(x!)\right\}\\
=-nE_\lambda\left\{ \frac{\ud}{\ud \lambda}
\left(-1+\frac{x}{\lambda}\right)\right\}\\
=-nE_\lambda\left(-\frac{x}{\lambda^2}\right) = \frac{n}{\lambda^2}E(X) =
\frac{n}{\lambda}\\
\end{array}
\end{equation*}

Therefore
$$Var_X(W(\bm X)) \geq \frac{1}{n/\lambda} = \frac{\lambda}{n}$$
In the example \ref{exampleBestEstimator1} we found that $Var(\bar X) =
\frac{\lambda}{n}$. It is equal to the Cramer-Rao lower bound. Therefore
$\bar X$ is the best unbiased estimator of $\lambda$.
\end{example}
\section{Asymptotic behaviour of estimators}
\begin{definition}
A sequence of estimators $W_n = W_n(X_1,\ldots,X_n)$ is consistent (tarkentuva,
konsistentti) sequence of estimators of $\theta$ if
$$W_n \xrightarrow{p} \theta$$
\end{definition}
\begin{example}
Let $X_1,X_2,\ldots$ be an i.i.d. sample from the Uniform($a$,$b$) population.
Let $W(\bm X)$ be the sample maximum $X_{(n)}$. As sample size increases,
$X_{(n)} \xrightarrow{p} b$, therefore the sample maximum is a consistent
estimator of population maximum $b$.
\end{example}
\begin{theorem}
If $W_n$ is a sequence of estimators of $\theta$ satisfying
\begin{itemize}
  \item[i)] $\lim_{n \to \infty} Var_\theta(W_n) = 0$
  \item[ii)] $\lim_{n \to \infty} Bias_\theta(W_n) = 0$
\end{itemize}
for every $\theta$, then $W_n$ is a consistent sequence of estimators of
$\theta$.
\end{theorem}
\begin{theorem} 
\label{theoremMLEConsistency1}
(Consistency of MLE's)

Let $X_1,X_2,\ldots$ be i.i.d. $f(x | \theta)$. Let $\hat\theta$ denote the MLE
of $\theta$ and let $\tau(\theta)$ be a continuous function of $\theta$. Under
mild regulatory conditions,
$$\tau(\hat\theta) \xrightarrow{p}\tau(\theta)$$
That is, $\tau(\hat\theta)$ is a consistent estimator of $\tau(\theta)$.
\begin{itemize}
  \item[$\to$] MLE estimators (or any continuous transformations of them) are
  consistent estimators of the population parameter (or the transformation of
  it).
\end{itemize}
\end{theorem}
\begin{theorem} 
\label{theoremAsEffMLE1}
(Asymptotic efficiency of MLE)

Under the conditions and definitions of theorem \ref{theoremMLEConsistency1}
$$\sqrt{n}\left( \tau(\hat \theta) - \tau(\theta) \right) \xrightarrow{d}
N(0, v(\theta))$$
where $v(\theta)$ is the Rao-Cramer lower bound
$$v(\theta) = \frac{[\tau'(\theta)]^2}{E_\theta\left\{ \left[ \frac{\ud}{\ud
\theta} ln(f(\bm X | \theta)) \right]^2 \right\}}$$
Therefore, we say that the MLE is asymptotically efficient (asymptoottisesti
tehokas).

\textbf{Proof: } See Casella \& Berger p. 472-473
\end{theorem}
\begin{example}
Let $X_1,X_2,\ldots$ be i.i.d. N($\mu$,$\sigma^2$). Consider the MLE of
$\sigma^2$
$$\hat\sigma^2 = \frac{1}{n}\sum(X_i - \bar X)^2$$ The Fishers information is
$$-E\left[\frac{\ud^2}{\ud(\sigma^2)^2} ln(L(\mu, \sigma^2 | \bm X)) \right] ~
\text{ (Lemma \ref{lemmaCB1})}$$
$$= \ldots = \frac{1}{2\sigma^4}$$
%TODO: Left out remark about ``showing this left as exerise''
$$\to Var(\sigma^2) = \frac{1}{1/2\sigma^4} = 2\sigma^4$$
Therefore the ML estimator $\hat \sigma^2$ fulfills
$$\sqrt{n}(\hat\sigma^2 - \sigma^2) \xrightarrow{d} N(0, 2\sigma^4)$$
R-script \verb#AsEffOfMLE.R# demonstrates this for a $N(0,
10\sqrt{(1/12)})$ for samples of size $n=(2,3,5,10,20,50,100,200,500)$
\end{example}
\begin{itemize}
  \item[\textbf{Note}] To use theorem \ref{theoremAsEffMLE1} with real data, one
  should know the true value of $\theta$, because $Var_\theta(\tau(\hat\theta))$
  is a function of $\theta$.
  \item The large sample variance of a ML estimator is given data $\bm x$ can be
  approximated as follows:
  \begin{itemize}
	\item Let
	$$I_n(\theta) = E_\theta\left[ \left( \frac{\ud}{\ud \theta} ln(L(\theta| \bm
	x)) \right)^2 \right]$$
	$$Var(\tau(\hat\theta)| \theta) \approx
	\frac{\left[\tau'(\theta)\right]^2}{I_n(\theta)} =
	\frac{\left[\tau'(\theta)\right]^2}{E_\theta\left[ -\frac{\ud^2}{\ud \theta^2}
	ln(L(\theta | \bm x))\right]}$$
	$$ \approx 
	\frac{\left[\tau'(\theta)\right]^2 |_{\theta = \hat\theta}}{
	-\frac{\ud^2}{\ud \theta^2} ln(L(\theta | \bm x))|_{\theta = \hat\theta}} 
	= \widehat{Var_\theta}(\tau(\hat\theta))$$
	\item It can be shown that this is a consistent estimator of
	$Var_\theta(\tau(\hat\theta))$
  \end{itemize}
  \item The generalization to vector-values $\bm \theta_{p \times 1} =
  \begin{bmatrix} \theta_1 \\ \theta_2 \\ \vdots \\ \theta_p \end{bmatrix}$
  \begin{itemize}
    \item The Fishers information matrix is a $p \times p$ matrix where element
    $(i,j)$ is defined as
    $$[I(\bm \theta)]_{ij} = E_\theta\left[ \frac{\ud}{\ud \bm \theta_i}ln(f(\bm
    X | \theta)) \frac{\ud}{\ud \bm \theta_j}ln(f(\bm X
    | \theta))\right]$$
    %TODO: Is the theta suposed to be vector inside the expected value in
    % previous equation?
    \item The MLE of $\bm \theta = \tau(\bm \theta)$ has the variance
    $[I(\bm \theta)]^{-1}$, which can be approximated by using same approach
    that was used in the case of scalar-valued $\theta$.
  \end{itemize}
\end{itemize}
\begin{example}
R-function \verb#mle()# computes this approximation numerically. Recall the
example \ref{exampleMLESpatial1} which used R-script \verb#MLESpatial.R#. 

Function \verb#ccov(sol)# returns this approximation
$$\bm \theta = \begin{bmatrix}
\mu \\ ln(\phi) \\ ln(\sigma^2)
\end{bmatrix}$$
$$\widehat{Var}(\hat \theta) = \begin{bmatrix}
7.25 & 0.64 & -0.58 \\
0.64 & 1.13 & -1.03 \\
-0.58 & -1.03 & 1.00
\end{bmatrix}$$
Using function \verb#summary(sol)# gives results
\begin{table}[h]
\caption{Correlations between the random variable pairs
of example \ref{exampleSpatialData1}}
\label{exampleSpatialData1Table1}
...
\renewcommand{\tabcolsep}{0.3cm}
\begin{tabular}{c|c|c|c}
& $\hat\theta$ & $sd(\hat \theta)$ & $\theta$ (true values) \\
\hline
$\mu$          & -1.87 & 2.69 & 0 \\
$\ln(\phi)$    & -2.03 & 1.06 & -0.69 \\
$ln(\sigma^2)$ & 2.34 & 1.00 & 1.38 \\
\end{tabular}
\end{table}
\end{example}
%TODO: Left out pages 6-8 from notes 12, since very little said about it.
\end{document}
