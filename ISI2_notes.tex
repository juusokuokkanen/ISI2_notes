...
| \theta ))\right]^2 \right\}}$$
\textbf{Note} The quantity $E_\theta\left\{ \left[ \frac{\ud}{\ud \theta} ln(f(X_i
| \theta ))\right]^2 \right\}$ is called the information number or Fisher's
information.
...
\frac{n}{\lambda}$$
Therefore
$$Var_X(W(\bm X)) \geq \frac{1}{n/\lambda} = \frac{n}{\lambda}$$
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    $(i,j)$ is defines as
    $$[I(\bm \theta)]_{ij} = E_\theta\left[ \frac{\ud}{\ud \theta_i}ln(f(\bm X
    | \theta)) \frac{\ud}{\ud \theta_j}ln(f(\bm X
    | \theta))\right]$$
    %TODO: Is the theta suposed to be vector inside the expected value in
    % previous equation?
    \item The MLE of $\bm \theta = \tau(\bm \theta)$ has the variance
    $[I(\theta)]^{-1}$, which can be approximated by using same approach that
    was used in the case of scalar-valued $\theta$.
  \end{itemize}
\end{itemize}
\begin{example}
